...
class FeedbackViewModel(private val repository: ElearningRepository) : ViewModel() {
    ...
    fun sendFeedback() {
        ...
        viewModelScope.launch {
            val result = repository.sendFeedback(message.value!!)
            if (result is Success) {
                val message = result.data
                _showMessageEvent.value = Event(message)
                this@FeedbackViewModel.message.value = null
            } else {
